...
if ($RubyVersion -like "3.1.*") {
    ...
    Write-Output "--- Installing Ruby $RubyVersion and NSYS2 using Chocolatey"
...
} elseif ($RubyVersion -like "3.4.*") {
    Write-Output "--- Installing Ruby $RubyVersion"
    choco install ruby --version $RubyVersion --force -y
    if (-not $?) { throw "Failed to install Ruby $RubyVersion." }
